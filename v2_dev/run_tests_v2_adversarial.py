"""
ClairDiag v2 — Task #007 Adversarial Validator

Перевіряє 4 осі + ранжування (must_be_in_top_n):
  1. safety_behavior       — safety floor спрацьовує правильно
  2. danger_exclusion      — небезпечні стани в TOP N (top + secondary + exclude)
  3. test_selection_adequacy — потрібні тести присутні
  4. output_coherence      — confidence і secondary адекватні

Використання:
  python run_tests_v2_adversarial.py
  python run_tests_v2_adversarial.py --verbose
  python run_tests_v2_adversarial.py --category false_anchor
"""
...
def get_top_n_conditions(result: dict) -> list:
    """
    Повертає всі умови які система явно згадує:
    top_hypothesis + secondary_hypotheses + exclude_priority
    Це і є наш TOP N простір.
    """
    covered = []
    if result.get("top_hypothesis"):
        covered.append(result["top_hypothesis"])
    covered.extend(result.get("secondary_hypotheses", []))
    covered.extend(result.get("exclude_priority", []))
    # дедуплікуємо зберігаючи порядок
    seen = set()
    unique = []
    for c in covered:
        if c not in seen:
...
# ──────────────────────────────────────────────
# ВІС 2: DANGER EXCLUSION + RANKING
# ──────────────────────────────────────────────

def eval_danger_exclusion(result: dict, expected: dict) -> tuple:
    exp        = expected.get("danger_exclusion", {})
    must_conds = exp.get("must_include_conditions", [])
    top_n      = exp.get("must_be_in_top_n", 3)

    if not must_conds:
        return True, "no expectation"

    top_conditions = get_top_n_conditions(result)
    top_n_slice    = top_conditions[:top_n]

    missing = []
    for cond in must_conds:
        if cond not in top_n_slice:
            missing.append(cond)

    if not missing:
        matched = [c for c in must_conds if c in top_n_slice]
        return True, f"conditions dans TOP {top_n}: {matched}"

    return False, (
        f"conditions manquantes dans TOP {top_n}: {missing} "
        f"(top_{top_n} actuel: {top_n_slice})"
    )
...
def eval_test_selection(result: dict, expected: dict) -> tuple:
    ...
    return True, f"tests OK: {sorted(actual_tests)}"
...
def eval_output_coherence(result: dict, expected: dict) -> tuple:
    ...
    if actual_sec < min_sec:
        return False, f"secondary count={actual_sec} < attendu {min_sec}"
...
def run(verbose: bool = False, category_filter: str = None) -> int:
    ...
    tests_path = os.path.join(base_dir, "tests_v2_adversarial.json")
    ...
    print("=" * 68)
    print("  ClairDiag v2 — Task #007 Adversarial Validation")
    print("=" * 68)
    ...
    for cat, rows in by_cat.items():
        ...
        for r in rows:
            ...
            if verbose:
                res      = r["result"]
                top_n    = get_top_n_conditions(res)
                tests    = [t["test"] for t in res.get("recommended_tests", [])]
                floor    = res.get("safety_floor", {}).get("triggered")
                print(f"         top={res.get('top_hypothesis')} secondary={res.get('secondary_hypotheses')}")
                print(f"         exclude={res.get('exclude_priority')}")
                print(f"         TOP_N_space={top_n} | tests={tests}")
                print(f"         floor={floor} conf={res.get('confidence_level')}")
    ...
    if n_fail_crit > 0:
        ...
        print()
        print("  ❌ Adversarial validation NOT passed.")
        print("=" * 68)
        return 1

    if n_partial > 0:
        print("\n  ⚠️  Adversarial validation PARTIAL — no critical failures.")
    else:
        print("\n  ✅ Adversarial validation PASSED — all cases clean.")
    print("=" * 68)
    return 0


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ClairDiag v2 Adversarial Validator")
    parser.add_argument("--verbose",  action="store_true")
    parser.add_argument("--category", type=str, default=None,
                        help="false_anchor|weak_signal_danger|mixed_benign_critical")
